"""
Azure Document Intelligence Layout adapter for PDFX-Bench.
Extracts text and tables using Azure AI Document Intelligence prebuilt-layout model.
"""
...
class AzureLayoutAdapter:
    """Adapter for Azure Document Intelligence Layout model."""
    ...
    def __init__(
        ...
        """
        Initialize Azure Document Intelligence Layout adapter.
        
        Args:
            endpoint: Azure Document Intelligence endpoint
            api_key: Azure API key
        """
        self.method = ExtractionMethod.AZURE_LAYOUT
    ...
    def _setup_client(self):
        ...
        try:
            ...
            logger.debug("Azure Document Intelligence Layout client configured successfully")
    ...
    def extract(
        ...
        **kwargs
        ...
        """
        Extract text and tables using Azure Document Intelligence Layout model.
        ...
        """
        logger.info(f"Starting Azure Layout extraction: {pdf_path}")
        
        with time_operation("azure_layout_extraction"):
            try:
                from azure.ai.documentintelligence.models import AnalyzeDocumentRequest
                ...
                if result.tables:
                    for table_idx, table in enumerate(result.tables):
                        ...
                        for cell in table.cells:
                            ...
                            # Create bounding box from polygon
                            bbox = self._create_bbox_from_polygon(
                                cell.polygon if hasattr(cell, 'polygon') else [],
                                800, 600  # Default page dimensions
                            )
                            ...
                            table_cell = TableCell(
                                ...
                                is_header=False,  # Azure doesn't provide header info
                                provenance=create_provenance(
                                    method=self.method.value,
                                    page=1,  # Azure doesn't provide page info for cells
                                    bbox=bbox,
                                    confidence=normalize_confidence(confidence, self.method),
                                    raw_data={
                        ...
                        if cells:
                            table_obj = Table(
                                cells=cells,
                                table_id=f"azure_layout_table_{table_idx}",
                                caption=None,  # Azure doesn't provide captions
                                provenance=create_provenance(
                                    method=self.method.value,
                                    page=1,  # Azure doesn't provide page info for tables
                                    bbox=create_bbox_from_coords(0, 0, 800, 600),
                                    confidence=1.0,
                                    raw_data={
                ...
                logger.info(f"Azure Layout extraction complete: {len(text_blocks)} text blocks, {len(tables)} tables")
                return document
    ...
    def is_available(endpoint: Optional[str] = None, api_key: Optional[str] = None) -> bool:
        """Check if Azure Layout adapter is available."""
        try:
            from azure.ai.documentintelligence import DocumentIntelligenceClient
            from azure.core.credentials import AzureKeyCredential
